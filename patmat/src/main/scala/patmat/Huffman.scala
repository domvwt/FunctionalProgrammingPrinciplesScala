...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def cond(letter: Char)(p: (Char, Int)): Boolean = p._1 == letter
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    def helper(freqs: List[(Char, Int)], checked: List[Leaf], unchecked: List[Leaf]): List[Leaf] = {
      freqs match {
        case Nil => unchecked
        case head :: _ =>
          if (unchecked.isEmpty) {
            helper(freqs.tail, List(), checked ::: List(Leaf(head._1, head._2)))
          } else if (head._2 < unchecked.head.weight) {
            helper(freqs.tail, List(), Leaf(head._1, head._2) :: unchecked)
          } else {
            helper(freqs, checked ::: List(unchecked.head), unchecked.tail)
          }
      }
    }
  ...
  def until[T](stopCondition: List[T] => Boolean, applyFunction: List[T] => List[T])
    ...
    if (stopCondition(listOfThings)) listOfThings
      ...
      until(stopCondition, applyFunction)(applyFunction(listOfThings))
    }
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def helper(treeNew: CodeTree, bits: List[Bit]): List[Char] = {
      treeNew match {
        case l: Leaf => l.char :: decode(tree, bits)
        case f: Fork => if (bits.head == 0) {
          helper(f.left, bits.tail)
        } else {
    ...
    if (bits.isEmpty) Nil else helper(tree, bits)
  }
    ...
    def helper(treeNew: CodeTree, text: List[Char]): List[Bit] = {
      treeNew match {
        ...
        case f: Fork => if (chars(f.left).contains(text.head)) {
          0 :: helper(f.left, text)
        } else {
          1 :: helper(f.right, text)
        }
      }
    }
